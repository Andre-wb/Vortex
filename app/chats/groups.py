...
async def list_topics(room_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    topics = (
        ...
        .order_by(Topic.is_pinned.desc(), Topic.last_message_at.desc().nullslast())
        .all()
    )
    return {
        "topics": [
...
async def create_topic(
    room_id: int, body: TopicCreate, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    _require_member(room_id, u.id, db)
...
async def update_topic(
    room_id: int, topic_id: int, body: TopicUpdate, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    if body.icon_emoji is not None:
        t.icon_emoji = body.icon_emoji
    if body.is_pinned is not None:
        t.is_pinned = body.is_pinned
    if body.is_closed is not None:
        t.is_closed = body.is_closed
    db.commit()
    return {"ok": True}


@router.delete("/{room_id}/topics/{topic_id}")
async def delete_topic(room_id: int, topic_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)):
    _require_admin(room_id, u.id, db)
    t = db.query(Topic).filter(Topic.id == topic_id, Topic.room_id == room_id).first()
    if not t:
        raise HTTPException(404, "Topic not found")
...
async def list_forum_threads(
    room_id: int, sort: str = "recent", u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    _require_member(room_id, u.id, db)
...
async def create_forum_thread(
    room_id: int, body: ForumThreadCreate, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    _require_member(room_id, u.id, db)
...
async def get_forum_thread(
    room_id: int, thread_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    _require_member(room_id, u.id, db)
...
async def update_forum_thread(
    ...
    if body.is_locked is not None:
        t.is_locked = body.is_locked
    if body.is_solved is not None:
        t.is_solved = body.is_solved
...
async def get_permissions(room_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)):
    _require_member(room_id, u.id, db)
...
async def set_permission(
    room_id: int, body: PermissionUpdate, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    _require_admin(room_id, u.id, db)
    if body.role:
        existing = (
...
async def list_automod_rules(room_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)):
    _require_admin(room_id, u.id, db)
...
async def create_automod_rule(
    room_id: int, body: AutoModRuleCreate, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    _require_admin(room_id, u.id, db)
    ...
    rule = AutoModRule(
        ...
        action=body.action,
        mute_duration_seconds=body.mute_duration_seconds,
        creator_id=u.id,
    )
    db.add(rule)
    db.commit()
...
async def delete_automod_rule(
    room_id: int, rule_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    _require_admin(room_id, u.id, db)
    rule = db.query(AutoModRule).filter(AutoModRule.id == rule_id, AutoModRule.room_id == room_id).first()
    if not rule:
...
async def list_user_slowmodes(room_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)):
    _require_admin(room_id, u.id, db)
    entries = db.query(UserSlowmode).filter(UserSlowmode.room_id == room_id).all()
    return {"slowmodes": [{"user_id": e.user_id, "cooldown_seconds": e.cooldown_seconds} for e in entries]}


@router.put("/{room_id}/slowmode/users")
async def set_user_slowmode(
    room_id: int, body: UserSlowmodeSet, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    _require_admin(room_id, u.id, db)
    existing = (
        db.query(UserSlowmode)
        .filter(
            UserSlowmode.room_id == room_id,
...
async def check_automod(room_id: int, user_id: int, text: str, member_role: RoomRole, db: Session) -> dict | None:
    """
    ...
    """
    rules = (
        db.query(AutoModRule)
        .filter(
            AutoModRule.room_id == room_id,
