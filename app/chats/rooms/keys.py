...
async def join_room(
    ...
    if online_count > 0:
        await _broadcast_key_request(room.id, u.id, u.x25519_public_key, u.kyber_public_key, u.kyber_public_key_sig)
...
async def provide_key(
    ...
    logger.info(
        f"Key provided for user {body.for_user_id} in room {room_id} by {u.username} (ws_delivered={delivered})"
    )
...
async def forget_my_key(
    ...
    if u.x25519_public_key and len(manager.get_online_users(room_id)) > 0:
        await _broadcast_key_request(room_id, u.id, u.x25519_public_key, u.kyber_public_key, u.kyber_public_key_sig)
...
async def list_room_invites(
    ...
    return {
        "invites": [
            ...
            for iv in invites
            if not _invite_expired(iv)  # истёкшие не ре-армим (O7 TTL)
        ]
    }
