...
def _get_default_category(space_id: int, db: Session) -> SpaceCategory:
    ...
    cat = db.query(SpaceCategory).filter(SpaceCategory.space_id == space_id).order_by(SpaceCategory.order_idx).first()
    return cat
...
async def my_spaces(
    ...
    return {"spaces": [{**_space_dict(s), "my_role": role_map.get(s.id)} for s in spaces]}


@router.get("/public")
async def public_spaces(db: Session = Depends(get_db)):
    """Публичные пространства для обзора."""
    spaces = db.query(Space).filter(Space.is_public.is_(True)).order_by(Space.member_count.desc()).limit(50).all()
...
async def get_space(
    ...
    # Категории
    cats = db.query(SpaceCategory).filter(SpaceCategory.space_id == space_id).order_by(SpaceCategory.order_idx).all()

    # Комнаты
    rooms = db.query(Room).filter(Room.space_id == space_id).order_by(Room.order_idx).all()
...
async def create_category(
    ...
    max_order = db.query(SpaceCategory).filter(SpaceCategory.space_id == space_id).count()
    cat = SpaceCategory(
        space_id=space_id,
