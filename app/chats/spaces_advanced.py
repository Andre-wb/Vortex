...
async def get_channel_permissions(
    space_id: int, room_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
...
async def set_channel_permission(
    space_id: int, body: PermissionOverride, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    if body.role:
        existing = (
            db.query(Permission)
            .filter(
                Permission.room_id == body.room_id,
...
async def create_sub_space(
    space_id: int, body: BaseModel | None = None, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    """Create a sub-space inside a parent space."""

    class SubSpaceBody(BaseModel):
        name: str = Field(..., min_length=1, max_length=100)
...
async def list_sub_spaces(space_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def set_onboarding(
    space_id: int, body: OnboardingUpdate, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    if body.welcome_message is not None:
        space.welcome_message = body.welcome_message
    if body.rules is not None:
        space.rules = body.rules
    if body.onboarding_roles is not None:
        space.onboarding_roles = json.dumps(body.onboarding_roles)
    db.commit()
...
@router.get("/discover")
async def discover_spaces(q: str = Query(default="", max_length=100), db: Session = Depends(get_db)):
    """Search and discover public spaces."""
    query = db.query(Space).filter(Space.is_public.is_(True))
    if q:
...
async def get_audit_log(
    ...
    _require_space_admin(space_id, u.id, db)
    logs = (
        db.query(AuditLog).filter(AuditLog.space_id == space_id).order_by(AuditLog.created_at.desc()).limit(limit).all()
    )
    return {
        "entries": [
            {
                ...
                "details": json.loads(line.details or "{}"),
...
async def list_emojis(space_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """List custom emojis for a space."""
    emojis = db.query(SpaceEmoji).filter(SpaceEmoji.space_id == space_id).all()
    return {"emojis": [{"id": e.id, "name": e.name, "image_url": e.image_url} for e in emojis]}
...
async def upload_emoji(
    ...
    existing = db.query(SpaceEmoji).filter(SpaceEmoji.space_id == space_id, SpaceEmoji.name == name.lower()).first()
    if existing:
...
async def delete_emoji(
    space_id: int, emoji_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
...
async def set_vanity_url(
    space_id: int, body: VanityUpdate, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
...
async def apply_template(
    space_id: int, template_id: str = Query(...), u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
