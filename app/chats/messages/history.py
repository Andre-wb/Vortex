...
async def send_history(room_id: int, user_id: int, db: Session) -> None:
    ...
    _pseudo_to_uid: dict[str, int] = {_compute_pseudo(room_id, rm.user_id): rm.user_id for rm in room_members}
    ...
    _uid_to_user: dict[int, User] = (
        {u.id: u for u in db.query(User).filter(User.id.in_(_member_uids)).all()} if _member_uids else {}
    )

    # Build uid→tag map for member tags
    _uid_to_tag: dict[int, tuple[str | None, str | None]] = {
        rm.user_id: (getattr(rm, "tag", None), getattr(rm, "tag_color", None)) for rm in room_members
    }

    # Pre-fetch all FileTransfer records for this room (batch, avoids N+1)
    _all_ft = (
        db.query(FileTransfer).filter(FileTransfer.room_id == room_id).order_by(FileTransfer.created_at.asc()).all()
    )
    ...
    _ft_by_name: dict[str, FileTransfer] = {name: fts[-1] for name, fts in _ft_by_name_list.items()}
    ...
    for m in messages:
        ...
        if m.msg_type == MessageType.SYSTEM and m.content_encrypted:
            with contextlib.suppress(Exception):
                ...
                if "question" in poll_data and "options" in poll_data:
                    _poll_sender_id = _pseudo_to_uid.get(m.sender_pseudo) if m.sender_pseudo else m.sender_id
        ...
        _resolved_sender_id = _pseudo_to_uid.get(m.sender_pseudo) if m.sender_pseudo else m.sender_id
        ...
        _sender_uid = _pseudo_to_uid.get(m.sender_pseudo) if m.sender_pseudo else m.sender_id
        read_by_others = any(
            rm.last_read_message_id and rm.last_read_message_id >= m.id
            for rm in room_members
            if rm.user_id != _sender_uid
        )
        if read_by_others:
            entry["status"] = "read"
    ...
    if pinned_id:
        ...
        if _pinned_msg:
            ...
            pinned_sender = (
                _pinned_msg.sender.display_name or _pinned_msg.sender.username if _pinned_msg.sender else None
            )
