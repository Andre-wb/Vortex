...
def _channel_dict(c: Room, db: Session, user_id: int | None = None) -> dict:
    ...
    if mon:
        d["monetization"] = {
            "is_paid": mon.is_paid,
            "price_display": mon.price_display,
...
async def create_channel(
    body: CreateChannelRequest, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    channel = Room(
        name=body.name,
...
async def join_channel(invite_code: str, u: User = Depends(get_current_user), db: Session = Depends(get_db)):
    channel = db.query(Room).filter(Room.invite_code == invite_code.upper(), Room.is_channel.is_(True)).first()
    if not channel:
        raise HTTPException(404, "Channel not found")
    existing = db.query(RoomMember).filter(RoomMember.room_id == channel.id, RoomMember.user_id == u.id).first()
    if existing:
    ...
    # Check paywall
    mon = (
        db.query(ChannelMonetization)
        .filter(ChannelMonetization.room_id == channel.id, ChannelMonetization.is_paid.is_(True))
        .first()
    )
    if mon:
        sub = (
    ...
    if _approval_enforced(channel):
        req = db.query(JoinRequest).filter(JoinRequest.room_id == channel.id, JoinRequest.user_id == u.id).first()
        if not req:
            db.add(JoinRequest(room_id=channel.id, user_id=u.id, x25519_pub=u.x25519_public_key))
            db.commit()
        return {"joined": False, "pending": True, "message": "Join request awaiting admin approval"}
...
async def channel_stats(channel_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    total_views = (
        ...
        .filter(PostView.message_id.in_(db.query(Message.id).filter(Message.room_id == channel_id)))
        .scalar()
        or 0
    )
    ...
    new_day = (
        ...
        .filter(RoomMember.room_id == channel_id, RoomMember.joined_at >= day_ago)
        .scalar()
        or 0
    )
    new_week = (
        db.query(func.count(RoomMember.id))
        .filter(RoomMember.room_id == channel_id, RoomMember.joined_at >= week_ago)
        .scalar()
        or 0
    )
    new_month = (
        db.query(func.count(RoomMember.id))
        .filter(RoomMember.room_id == channel_id, RoomMember.joined_at >= month_ago)
        .scalar()
        or 0
    )
    ...
    views_today = (
        ...
        .filter(
            ...
            PostView.message_id.in_(db.query(Message.id).filter(Message.room_id == channel_id)),
        )
        .scalar()
        or 0
    )
    ...
    total_reactions = (
        ...
        .filter(PostReaction.message_id.in_(db.query(Message.id).filter(Message.room_id == channel_id)))
        .scalar()
        or 0
    )
    ...
    total_donations = (
        db.query(func.count(ChannelDonation.id)).filter(ChannelDonation.room_id == channel_id).scalar() or 0
    )
    active_subs = (
        db.query(func.count(ChannelSubscription.id))
        .filter(
...
async def record_view(
    channel_id: int, message_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    existing = db.query(PostView).filter(PostView.message_id == message_id, PostView.user_id == u.id).first()
    if not existing:
        ...
        "comments": [
            {
                ...
                "content_encrypted": m.content_encrypted.hex() if m.content_encrypted else "",
...
async def schedule_post(
    channel_id: int, body: SchedulePostRequest, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
...
async def list_scheduled(channel_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return {
        "scheduled": [{"id": m.id, "scheduled_at": m.scheduled_at.isoformat() if m.scheduled_at else ""} for m in msgs]
    }
...
async def discover_channels(
    q: str = Query(default="", max_length=100), category: str = Query(default=""), db: Session = Depends(get_db)
):
...
async def set_monetization(
    channel_id: int, body: SetMonetizationRequest, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    if existing:
        ...
        db.add(
            ChannelMonetization(
                ...
                donations_enabled=body.donations_enabled,
            )
        )
    db.commit()
    return {"ok": True}
...
async def subscribe_to_paid(
    channel_id: int, body: SubscribeRequest, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    mon = (
        db.query(ChannelMonetization)
        .filter(ChannelMonetization.room_id == channel_id, ChannelMonetization.is_paid.is_(True))
        .first()
    )
    if not mon:
    ...
    duplicate = db.query(ChannelSubscription).filter(ChannelSubscription.tx_hash == body.tx_hash).first()
    if duplicate:
    ...
    sub = ChannelSubscription(
        ...
        user_id=u.id,
        tx_hash=body.tx_hash,
        amount=verification.amount_received or body.amount or mon.price_display,
        expires_at=datetime.now(timezone.utc) + timedelta(days=30),
    )
    db.add(sub)
    # Also add as member if not already
    existing = db.query(RoomMember).filter(RoomMember.room_id == channel_id, RoomMember.user_id == u.id).first()
    if not existing:
...
async def donate_to_channel(
    channel_id: int, body: DonateRequest, u: User = Depends(get_current_user), db: Session = Depends(get_db)
):
    """Send donation to channel author. All money goes to author's wallet (0% fee)."""
    mon = (
        db.query(ChannelMonetization)
        .filter(ChannelMonetization.room_id == channel_id, ChannelMonetization.donations_enabled.is_(True))
        .first()
    )
    if not mon:
    ...
    # Prevent replay: same tx_hash cannot be used twice
    duplicate = db.query(ChannelDonation).filter(ChannelDonation.tx_hash == body.tx_hash).first()
    if duplicate:
        raise HTTPException(409, "This transaction has already been used")
    ...
    don = ChannelDonation(
        ...
        user_id=u.id,
        tx_hash=body.tx_hash,
        amount=verification.amount_received or body.amount,
...
async def list_donations(channel_id: int, u: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """List donations (owner only)."""
    channel = db.query(Room).filter(Room.id == channel_id).first()
    if not channel or channel.creator_id != u.id:
        raise HTTPException(403, "Owner only")
    dons = (
        db.query(ChannelDonation)
        .filter(ChannelDonation.room_id == channel_id)
        .order_by(ChannelDonation.created_at.desc())
        .limit(100)
        .all()
    )
    return {
...
async def list_subscribers(
    ...
    if q:
        user_ids = [
            ...
            .filter(User.username.ilike(f"%{q}%") | User.display_name.ilike(f"%{q}%"))
            .all()
        ]
