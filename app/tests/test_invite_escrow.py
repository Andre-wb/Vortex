...
def _set_invite_expiry(room_id, invite_pub, when):
    ...
    db = SessionLocal()
    try:
        iv = db.query(RoomInvite).filter(
            RoomInvite.room_id == room_id, RoomInvite.invite_pub == invite_pub).first()
...
def _escrow_body(hybrid=False) -> dict:
    ...
    if hybrid:
        body.update({
            "hybrid": True, "x25519_ephemeral_pub": secrets.token_hex(32),
            "kyber_ciphertext": secrets.token_hex(1088), "ciphertext": secrets.token_hex(60),
...
def _fetch(client, headers, room_id, invite_pub):
    return client.get(f"/api/rooms/{room_id}/invite-escrow",
                      params={"invite_pub": invite_pub}, headers=headers)
...
def test_store_accepts_exact_client_body_shape(client):
    ...
    inv = next((i for i in _list_invites(client, o_h, room_id)
                if i["invite_pub"] == body["invite_pub"]), None)
...
def _escrow_row_exists(room_id, invite_pub):
    ...
    db = SessionLocal()
    try:
        return db.query(RoomInviteEscrow).filter(
            RoomInviteEscrow.room_id == room_id,
            RoomInviteEscrow.invite_pub == invite_pub).first() is not None
    finally:
        db.close()
...
def test_rewrap_does_not_extend_ttl(client):
    ...
    body2 = dict(body); body2["ciphertext"] = secrets.token_hex(60)
...
def test_revoke_removes_invite_and_escrow(client):
    ...
    body = _escrow_body()
    _store(client, o_h, room_id, body)
    r = client.delete(f"/api/rooms/{room_id}/invite-escrow",
                      params={"invite_pub": body["invite_pub"]}, headers=o_h)
...
def test_rotate_deletes_escrow_but_invite_persists(client):
    ...
    row = next((i for i in _list_invites(client, o_h, room_id)
                if i["invite_pub"] == body["invite_pub"]), None)
