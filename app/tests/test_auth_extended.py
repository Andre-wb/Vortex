...
class TestTwoFA:
    ...
    def test_2fa_setup(self, client, logged_user):
        r = client.post("/api/authentication/2fa/setup", headers=logged_user["headers"])
    ...
    def test_2fa_status(self, client, logged_user):
        r = client.get("/api/authentication/2fa/status", headers=logged_user["headers"])
        assert r.status_code == 200
        data = r.json()
...
class TestTokenRefresh:
    ...
    def test_logout_clears_tokens(self, client, logged_user):
        r = client.post("/api/authentication/logout", headers=logged_user["headers"])
