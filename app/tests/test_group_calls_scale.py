...
class TestGroupCallQuality:
    ...
    def test_quality_profile(self, n):
        ...
        # Assertions
        assert q["bandwidth_ok"], (
            f"{n} participants: bandwidth {q['total_per_user_mbps']} Mbps exceeds {AVG_BANDWIDTH_MBPS} Mbps"
        )
...
class TestGroupCallAPI:
    ...
    def test_start_and_join_group_call(self, client, n):
        ...
        h_creator = login_user(client, users[0]["username"], users[0]["password"])
        r = client.post(
            f"/api/group-calls/{room['id']}/start",
            json={
...
class TestQualitySummary:
    ...
    def test_print_quality_table(self):
        ...
        print(f"{'=' * 110}")
        print(
            f"  {'N':>4} | {'Topology':>5} | {'Video':>8} | {'Res':>5} | {'FPS':>3} | "
        ...
        for n in sizes:
            q = _calc_quality(n)
            print(
                f"  {n:>4} | {q['topology']:>5} | {q['video_profile']:>8} | "
