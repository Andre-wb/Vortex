...
class TestBandwidthWithSimulcast:
    ...
    def test_bandwidth_comparison_table(self):
        ...
        print(f"{'=' * 95}")
        print(
            f"  {'N':>4} | {'Mesh (Mbps)':>12} | {'SFU basic':>12} | {'SFU+Sim+VP':>12} | "
        ...
        for n in sizes:
            ...
            visible = min(n - 1, 4)
            hidden = max(n - 1 - visible - 1, 0)
            download_sv = (2_500_000 + 32_000) + visible * (600_000 + 32_000) + hidden * 32_000
            ...
            print(
                f"  {n:>4} | {mesh_per_user:>10.1f}  | {sfu_basic:>10.1f}  | "
...
class TestScaleQuality:
    ...
    def test_client_bandwidth_under_limit(self, n):
        ...
        if n > 500:
            ...
            assert bw["total_mbps"] < 25, f"{n} participants: client needs {bw['total_mbps']} Mbps"
    ...
    def test_audio_bridge_optimization(self, n):
        ...
        assert total_bridged < 10, f"{n} with audio bridge: {total_bridged:.1f} Mbps should be under 10"
