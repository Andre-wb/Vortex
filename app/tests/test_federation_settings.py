...
class TestGossipProtocol:
    ...
    def test_gossip_node_left_unknown(self, client: SyncASGIClient, logged_user: dict):
        ...
        r = client.post(
            "/api/federation/gossip/node-left",
            json={
                "node_id": secrets.token_hex(16),
...
class TestCodeVerification:
    ...
    def test_code_manifest_file_count(self, client: SyncASGIClient, logged_user: dict):
        ...
        r = client.post("/api/federation/code-manifest", headers={**logged_user["headers"], **fed_proof_headers()})
        assert r.status_code == 200
        body = r.json()
...
class TestTokenValidation:
    ...
    def test_validate_invalid_token(self, client: SyncASGIClient, logged_user: dict):
        ...
        r = client.post(
            "/api/federation/validate-token",
            json={
                "node_id": "fake-node",
                "token": "fake-token-1234",
            },
            headers={**logged_user["headers"], "X-Federation-Proof": make_federation_proof("fake-node")},
        )
        assert r.status_code == 200
    ...
    def test_validate_token_for_real_node(self, client: SyncASGIClient, logged_user: dict):
        ...
        nid = secrets.token_hex(16)
        tag = random_str()
        _register_node_via_handshake(client, h, node_id=nid, url=f"https://token-test-{tag}.example.com:8443")
        r = client.post(
            "/api/federation/validate-token",
            json={
                "node_id": nid,
