...
def _bundle_with_pqopks(account_ed, n_pqopk=2, base_id=1000):
    ...
    return {
        ...
        "one_time_kyber_prekeys": [
            {"key_id": base_id + i, "public_key": secrets.token_bytes(1184).hex()}
            for i in range(n_pqopk)
        ],
    }
...
def _register(tc):
    ...
    tc.post("/api/authentication/register", json={
        "username": f"po_{tag}", "password": _PW, "display_name": f"PO {tag}",
...
def _claim(tc, csrf, uid, want_kyber):
    return tc.post(f"/api/keys/prekeys/{uid}/claim-opk",
                   json={"want_kyber": want_kyber}, headers={"X-CSRF-Token": csrf}).json()
...
class TestKyberOneTimePrekey:
    ...
    def test_claim_want_kyber_returns_and_consumes(self, warn_only):
        with TestClient(app, raise_server_exceptions=False) as tc:
            u = _register(tc); csrf = _login(tc, u)
