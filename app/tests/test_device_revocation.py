...
def _bundle(ik):
    ...
    return {
        "identity_key": ik, "signed_prekey": spk.hex(),
        "signed_prekey_sig": ed.sign(spk).hex(), "signed_prekey_id": 1,
...
def _register(tc):
    ...
    tc.post("/api/authentication/register", json={
        "username": f"rv_{tag}", "password": _PW, "display_name": f"RV {tag}",
...
def _publish(tc, csrf, ik, cid):
    tc.post("/api/keys/prekeys/publish", json=_bundle(ik),
            headers={"X-CSRF-Token": csrf, "X-Device-Id": cid})
...
def test_logout_device_removes_prekey_bundle(warn_only):
    with TestClient(app, raise_server_exceptions=False) as tc:
        ...
        r = tc.delete(f"/api/authentication/devices/{dev_b['id']}",
                      headers={"X-CSRF-Token": csrf_a, "X-Device-Id": cid_a})
