...
class TestECIESFullCycle:
    ...
    def test_ecies_for_client_matches_browser_dialect(self):
        ...
        browser_key = HKDF(algorithm=hashes.SHA256(), length=32, salt=None, info=b"vortex-session").derive(shared)
        ...
        node_key = HKDF(
            algorithm=hashes.SHA256(), length=32, salt=b"".join(sorted([eph_pub, pub_bytes])), info=b"vortex-session"
        ).derive(shared)
