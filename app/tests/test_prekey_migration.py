...
def test_rebuild_adds_device_id_and_preserves_rows(tmp_path):
    ...
    with engine.begin() as conn:
        ...
        rows = conn.execute(
            text("SELECT user_id, device_id, supports_v2 FROM prekey_bundles ORDER BY user_id")
        ).fetchall()
...
def test_rebuild_is_idempotent(tmp_path):
    ...
    with engine.begin() as conn:
        ...
        orphan = conn.execute(text("SELECT 1 FROM sqlite_master WHERE name='prekey_bundles_new'")).fetchone()
...
def test_rebuild_noop_when_table_absent(tmp_path):
    ...
    with engine.begin() as conn:
        ...
        assert conn.execute(text("SELECT 1 FROM sqlite_master WHERE name='prekey_bundles'")).fetchone() is None
