...
SCHEMA: list[dict] = [
    ...
    _field("PORT", "int", "Network", "Port", default="9000"),
    _field(
        "NETWORK_MODE",
        "select",
        "Network",
    ...
    _field("DEVICE_NAME", "str", "Network", "Device name"),
    _field(
        "ANNOUNCE_URL",
        "str",
        "Network",
    ...
    _field("CONTROLLER_URL", "str", "Network", "Controller URL"),
    _field("CONTROLLER_PUBKEY", "str", "Network", "Controller pubkey (hex)"),
    _field("CONTROLLER_FALLBACK_URLS", "textarea", "Network", "Fallback controllers"),
    # ── Database
    _field(
        "DATABASE_URL",
        "str",
    ...
    _field("DB_PATH", "str", "Database", "SQLite path", default="vortex.db"),
    # ── Security
    _field(
        "EPHEMERAL_MODE",
        "bool",
        "Security",
        "Ephemeral mode (RAM-only DB)",
        default="false",
    ...
    _field("HASH_IPS", "bool", "Security", "Hash IPs before storage", default="true"),
    _field(
        "METADATA_PADDING",
        "bool",
        "Security",
    ...
    _field(
        ...
        "bool",
        "Federation",
        "BMP anonymous delivery",
        default="true",
        desc="Blind Mailbox Pattern — сервер не знает кому шлёт сообщение.",
    ),
    _field("FEDERATION_ENABLED", "bool", "Federation", "Federation enabled", default="true"),
    # ── AI / integrations
    _field("AI_ENABLED", "bool", "AI", "AI assistant", default="true"),
    _field(
        "AI_PROVIDER", "select", "AI", "AI provider", default="auto", options=["auto", "ollama", "openai", "anthropic"]
    ),
    _field("OLLAMA_URL", "str", "AI", "Ollama URL", default="http://localhost:11434"),
    _field("OLLAMA_MODEL", "str", "AI", "Ollama model", default="llama3"),
    _field("AI_API_KEY", "password", "AI", "External API key", secret=True),
    _field("AI_API_URL", "str", "AI", "External API URL"),
    _field("AI_MODEL", "str", "AI", "External model name"),
    _field("TRANSLATE_ENABLED", "bool", "AI", "Translation", default="false"),
    _field("TRANSLATE_URL", "str", "AI", "Translate backend", default="http://localhost:5000"),
    # ── Operator / monetization
    _field("MIRROR_ENABLED", "bool", "Operator", "Backup mirror enabled", default="false"),
    _field("MIRROR_CONTROLLER_URL", "str", "Operator", "Mirror controller URL"),
    _field("AUTOCOMPOUND_ENABLED", "bool", "Operator", "Autocompound rewards", default="false"),
    _field("AUTOCOMPOUND_THRESHOLD", "str", "Operator", "Autocompound threshold (SOL)", default="1.0"),
    _field("VERSION_PIN", "bool", "Operator", "Pin version (no update banner)", default="false"),
    # ── Accessibility / wizard
    _field("TOUR_COMPLETED", "bool", "Wizard", "Guided tour dismissed", default="false", restart=False),
]
...
async def settings_get(request: Request) -> dict:
    ...
    for s in SCHEMA:
        ...
        display = "•" * 10 if s["secret"] and cur else cur
...
async def settings_patch(body: PatchBody, request: Request) -> dict:
    ...
    for k, v in body.changes.items():
        ...
        if spec["type"] == "bool":
            ...
            try:
                ...
                raise HTTPException(400, f"{k}: expected int, got {v!r}") from None
    ...
    requires_restart = any(by_key.get(k, {}).get("requires_restart", True) for k in updates)
    return {
        "ok": True,
        "updated": len(updates),
