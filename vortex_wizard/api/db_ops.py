...
async def migrate_run(body: MigrateBody, request: Request) -> dict:
    ...
    try:
        ...
        raise HTTPException(500, "psycopg2 not installed — pip install psycopg2-binary") from None
    ...
    try:
        ...
        raise HTTPException(502, f"postgres connect failed: {e}") from None
    ...
    try:
        ...
        for (table,) in sq.execute("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'"):
            if table.startswith("alembic_"):
                continue
            cols = [r["name"] for r in sq.execute(f'PRAGMA table_info("{table}")')]
            if not cols:
                continue
            rows = sq.execute(f'SELECT {",".join(cols)} FROM "{table}"').fetchall()  # noqa: S608
            if not rows:
                report[table] = 0
                continue
            quoted_cols = ",".join(f'"{c}"' for c in cols)
            sql = f'INSERT INTO "{table}" ({quoted_cols}) VALUES %s ON CONFLICT DO NOTHING'  # noqa: S608
...
async def schema_diff(request: Request) -> dict:
    ...
    try:
        ...
        raise HTTPException(500, f"cannot import app.base: {e}") from None
    ...
    if is_sq and db_path and db_path.is_file():
        ...
        try:
            ...
            raise HTTPException(500, f"schema introspection failed: {e}") from None
...
async def central_audit_nodes(request: Request) -> dict:
    ...
    if not p.is_file():
        return {"nodes": []}
    try:
        return {"nodes": json.loads(p.read_text()).get("nodes", [])}
    except Exception:
        return {"nodes": []}
...
class FailoverRegion(BaseModel):
    ...
    fallbacks: list[str] = Field(default_factory=list, description="ordered list of fallback base URLs")
    ...
    for r in state.get("regions", []):
        ...
        for url in candidates:
            try:
                ...
                continue
        return {"region": region, "resolved": None, "healthy": False, "tried": candidates}
