...
async def pull_secret(body: PullBody, request: Request) -> dict:
    ...
    try:
        ...
        raise HTTPException(502, f"provider fetch failed: {e}") from None
    ...
    _record_access(state, action="pull", env_key=body.env_key, provider=prov["id"])
    _save_state(env_file, state)
...
async def _fetch_from_provider(prov: dict) -> str:
    ...
    if t == "gcp":
        ...
        creds = service_account.Credentials.from_service_account_info(json.loads(cfg["service_account_json"]))
...
class CeremonyConfigBody(BaseModel):
    ...
    admin_pubkeys: list[str] = Field(..., description="Ed25519 pubkeys of admin signers, hex")
...
async def ceremony_start(body: StartCeremonyBody, request: Request) -> dict:
    ...
    _save_state(env_file, state)
    return {"ok": True, "ceremony_id": req_id, "need_approvals": state["ceremony"]["required_approvals"]}
...
async def ceremony_approve(body: ApproveBody, request: Request) -> dict:
    ...
    try:
        try:
            ...
            ok = _vc.verify_signature(bytes.fromhex(body.pubkey), msg, bytes.fromhex(body.signature))
        except Exception:
            from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PublicKey

            Ed25519PublicKey.from_public_bytes(bytes.fromhex(body.pubkey)).verify(bytes.fromhex(body.signature), msg)
            ok = True
    except Exception:
