...
def _block_hashes(data: bytes) -> list[str]:
    return [hashlib.sha256(data[i : i + _BLOCK_SIZE]).hexdigest() for i in range(0, len(data), _BLOCK_SIZE)]
...
async def incremental_upload(request: Request) -> dict:
    ...
    if mpath.is_file():
        with contextlib.suppress(Exception):
            prev_hashes = json.loads(mpath.read_text()).get("hashes", [])
...
class ControllersBody(BaseModel):
    controllers: list[str] = Field(..., min_length=1, max_length=8, description="List of controller base URLs")
...
async def list_controllers(request: Request) -> dict:
    ...
    return {
        "controllers": [env.get("CONTROLLER_URL", "")]
...
async def job_multi_controller_backup(env_file: Path) -> dict:
    ...
    if not primary:
        return {"skipped": True, "message": "no primary controller"}
    ...
    for ctrl in [primary, *mirrors]:
        try:
            ...
            report[ctrl] = f"err:{e}"
    return {"message": f"pushed to {sum(1 for v in report.values() if v == 'ok')}/{len(report)}", "report": report}
...
async def ipfs_upload(request: Request) -> dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as c:
            r = await c.post(url + "/api/v0/add", files=files, params={"pin": str(env.get("IPFS_PIN", "true")).lower()})
        ...
        raise HTTPException(503, f"IPFS daemon unreachable: {e}") from None
...
async def ipfs_history(request: Request) -> dict:
    ...
    if not p.is_file():
        return {"entries": []}
    try:
        return {"entries": json.loads(p.read_text())}
    except Exception:
        return {"entries": []}
...
async def s3_upload(request: Request) -> dict:
    ...
    try:
        ...
        raise HTTPException(500, "boto3 not installed — pip install boto3") from None
