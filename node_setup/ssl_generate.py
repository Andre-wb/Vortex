...
def generate_self_signed(
    ...
    ca_key = rsa.generate_private_key(public_exponent=65537, key_size=4096, backend=backend)
    ca_name = x509.Name(
        [
    ...
    srv_key = rsa.generate_private_key(public_exponent=65537, key_size=2048, backend=backend)
...
def generate_with_mkcert(cert_dir: Path, hostname: str = "") -> SSLResult:
    ...
    result = subprocess.run(
        [mkcert_bin, "-cert-file", str(cert_path), "-key-file", str(key_path), *domains],
        capture_output=True,
        text=True,
    )
    if result.returncode != 0:
        return SSLResult(ok=False, cert="", key="", ca="", message=f"mkcert ошибка: {result.stderr}", trusted=False)
...
def generate_letsencrypt(
    ...
    if result.returncode != 0:
        return SSLResult(
            ok=False, cert="", key="", ca="", message=f"certbot ошибка: {result.stderr[:400]}", trusted=False
        )
    ...
    if not cert_path.exists() or not key_path.exists():
        return SSLResult(
            ok=False, cert="", key="", ca="", message=f"certbot: файлы не найдены в {live_dir}", trusted=False
        )
...
def use_manual_cert(cert_src: str, key_src: str, cert_dir: Path) -> SSLResult:
    ...
    try:
        ...
        return SSLResult(
            ok=True, cert=str(cert_path), key=str(key_path), ca="", message="Сертификат скопирован", trusted=True
        )
    except Exception as e:
        return SSLResult(ok=False, cert="", key="", ca="", message=f"Ошибка копирования: {e}", trusted=False)
