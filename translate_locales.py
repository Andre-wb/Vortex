...
def _discover_locale_dir():
    ...
    for c in candidates:
        if os.path.isdir(c) and os.path.isfile(os.path.join(c, "en.json")):
            return c
    raise SystemExit("Could not find a locales directory with en.json. Expected one of: locales/, static/locales/")
...
def _protect_value(text, section):
    ...
    text = re.sub(r"<code>[^<]*</code>", lambda m: _add(m.group(0)), text)

    # Layer 2: remaining standalone HTML tags
    text = re.sub(r"</?(strong|em|br|a|span|div|ul|li|p)\b[^>]*>", lambda m: _add(m.group(0)), text)
    ...
    text = re.sub(r"\b[A-Z][A-Z0-9]{1,}(?:[-/][A-Z0-9]+)*\b", lambda m: _add(m.group(0)), text)
        ...
        for term in _GRAVITIX_TERMS:
            ...
            if len(term) <= 3 and term.isalpha():
                ...
                if re.search(pattern, text):
                    text = re.sub(pattern, lambda m, t=term: _add(t), text)
            else:
                while term in text:
                    text = text.replace(term, _add(term), 1)
...
def batch_translate(strings, target_lang, retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            restored = []
            for orig, translated in zip(strings, parts, strict=False):
            ...
            if attempt < retries - 1:
                ...
                if is_network and attempt >= 1:
                    print(f"    network/rate-limit; sleeping {delay}s before retry...", flush=True)
                time.sleep(delay)
            else:
                err_preview = str(e)[:160].replace("\n", " ")
                print(
                    f"    ERROR translating batch ({len(strings)} strings, {_joined_len(strings)} chars): {err_preview}"
                )
                return strings  # fallback to original
...
def _check_supported(google_code):
    ...
    for attempt in range(3):
        try:
            ...
            if ("language" in msg and "not" in msg) or "invalid" in msg or "not supported" in msg:
                return False
...
def process_locale(fname, en_data):
    ...
    if google_code not in _supported_cache:
        ...
        if probe is None:
            print(f"  RATE-LIMITED probing {google_code} — waiting 60 s then retrying...", flush=True)
            time.sleep(60)
            probe = _check_supported(google_code)
    ...
    strings = []
    protection_maps = []
    for section, _key, value in fallbacks:
    ...
    def flush_to_disk(n_trans_so_far: int):
        ...
        for i in range(n_trans_so_far):
            if i >= len(fallbacks) or i >= len(translated):
                break
            ...
            if trans and trans != orig and _set_by_path(locale_data, section, key_path, trans):
                written += 1
        if written > 0:
    ...
    def run_batch(current_batch):
        ...
        if is_fail:
            ...
            consecutive_net_fails = 0
            ok_count = sum(1 for a, b in zip(current_batch, result, strict=False) if a != b)
            status = f"ok {ok_count}/{bs}"
        elapsed_min = (time.monotonic() - locale_start) / 60
        print(
            f"  [{_ts()}] {fname} batch {bn}/{total_batches}  "
    ...
    for s in strings:
        ...
        if batch and (len(batch) >= BATCH_SIZE or projected > MAX_BATCH_CHARS):
            ...
            if consecutive_net_fails >= MAX_CONSECUTIVE_NET_FAILS:
                print(
                    f"  [{_ts()}] {fname}: {MAX_CONSECUTIVE_NET_FAILS} "
            ...
            if batches_since_flush >= FLUSH_EVERY_BATCHES:
                ...
                print(f"  [{_ts()}] {fname} → flushed to disk ({w}/{len(fallbacks)} cumulative)", flush=True)
                batches_since_flush = 0
    ...
    print(f"  [{_ts()}] {fname}: {changed}/{len(fallbacks)} strings translated in {total_min:.1f}m{suffix}", flush=True)
    return changed
...
def main():
    ...
    all_files = [f.replace(".json", "") for f in os.listdir(LOCALE_DIR) if f.endswith(".json") and f != "en.json"]
