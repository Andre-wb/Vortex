#!/usr/bin/env python3
"""Генератор golden-векторов Trojan Python ↔ Rust.

Повторный запуск даёт байт-в-байт идентичный файл.

Запуск (из корня репозитория):
    python scripts/gen_trojan_parity_vectors.py
"""
...
OUT_PATH = os.path.join(ROOT, "app", "tests", "vectors", "trojan_parity.json")

_spec = importlib.util.spec_from_file_location(
    "trojan_parity_reference",
    os.path.join(ROOT, "app", "tests", "trojan_parity_reference.py"),
)
_reference = importlib.util.module_from_spec(_spec)
sys.modules["trojan_parity_reference"] = _reference
