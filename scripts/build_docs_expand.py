    ...
    for k in list(node.keys()):
        ...
        if not node.get(desc_key):
            if base_para:
                ...
                node[desc_key] = (
                    f"{heading_text} — part of the {chapter_label} chapter. {CHAPTER_META_TEMPLATE['desc_suffix']}"
                )
        if not node.get(mech_key):
            node[mech_key] = f'Implementation overview for "{heading_text}". ' + CHAPTER_META_TEMPLATE["mech_suffix"]
