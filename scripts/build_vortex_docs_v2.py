...
GLOSSARY_CHAPTER = {
    ...
    **glossary(
        ("AES-GCM", "Authenticated encryption mode. 256-bit key, 96-bit nonce, 128-bit tag."),
        ("Alembic", "Database migration tool for SQLAlchemy. Linear revision history."),
        ("APNs", "Apple Push Notification service. Used for iOS push."),
        ("Argon2id", "Memory-hard password hash. Vortex params: m=64MiB, t=3, p=4."),
        ("AAD", "Additional Authenticated Data. In AES-GCM, data included in the tag but not encrypted."),
        (
            "BLAKE3",
            "Fast cryptographic hash. Vortex uses it for file integrity; column is still named sha256 for migration compatibility.",
        ),
        ("BMP", "Blind Mailbox Protocol. Store-and-forward through the gossip mesh."),
        ("Canary", "Signed daily file proving the release key still controls the deployment."),
        ("CBOR", "Concise Binary Object Representation. RFC 8949. Encoding used for E2E message payloads."),
        ("Controller", "Vortex's separate attestation process. Publishes `/v1/integrity` and entry URLs."),
        ("Double Ratchet", "Signal's per-session forward-secrecy protocol."),
        ("DGA", "Domain Generation Algorithm. Deterministic fallback domain generator."),
        ("ECH", "Encrypted Client Hello. Hides SNI on TLS 1.3."),
        ("Ed25519", "EdDSA signature scheme over edwards25519. 32-byte pubkey, 64-byte signature."),
        ("Envelope", "The wrapper around ciphertext carrying nonce + metadata."),
        ("FCM", "Firebase Cloud Messaging. Used for Android push."),
        ("Federation", "Nodes trusting each other to deliver messages to users on other nodes."),
        ("FTS5", "SQLite's full-text search virtual table."),
        ("Gossip", "Periodic peer-list exchange between nodes."),
        ("Gravitix", "Vortex's domain-specific language for writing bots."),
        ("HKDF", "HMAC-based Key Derivation Function. RFC 5869. SHA-256 variant used."),
        ("HMAC", "Hash-based Message Authentication Code. RFC 2104."),
        ("ICE", "Interactive Connectivity Establishment. WebRTC's NAT traversal."),
        ("JWT", "JSON Web Token. Vortex uses HS256-signed JWTs for session auth."),
        ("Kyber-768", "ML-KEM-768 post-quantum KEM. FIPS 203."),
        ("Mailbox", "A recipient's inbox for BMP blobs. ID derived from recipient's pubkey."),
        ("Nonce", "Number used once. 96-bit random value for AES-GCM."),
        ("Node", "A Python process running the Vortex back-end. Stores ciphertext + fans out events."),
        ("OHTTP", "Oblivious HTTP. RFC 9458. Relay-assisted anonymous HTTP."),
        ("Onion service", "Tor hidden service. Every Vortex node exposes itself as .onion."),
        ("Prekey", "Ephemeral pubkey published ahead of time to bootstrap new DMs."),
        ("Ratchet", "One step in the Double Ratchet chain. Each message ratchets forward."),
        ("Reality", "Stealth transport that masquerades as a real big-tech site."),
        ("Room key", "32-byte symmetric key shared among room members. Rotated on membership change."),
        ("Sealed push", "Push payload encrypted so the provider (Apple/Google) can't read it."),
        ("SFU", "Selective Forwarding Unit. Mixes media in group calls."),
        ("Snowflake", "Tor transport using volunteer browsers as WebRTC relays."),
        ("TLS 1.3", "RFC 8446. Transport used by every HTTPS and WSS connection."),
        ("TOTP", "RFC 6238. Time-based one-time password for 2FA."),
        ("TURN", "Traversal Using Relays around NAT. Coturn handles it in Vortex."),
        ("VAPID", "RFC 8292. Voluntary Application Server Identification for Web Push."),
        ("WAF", "Web Application Firewall. Vortex ships a built-in one."),
        ("X25519", "Elliptic-curve Diffie-Hellman over Curve25519. RFC 7748."),
    ),
}
