...
def route(
    ...
    return {
        ...
        "h1_a": purpose_desc,
        "h1_b": purpose_mech
        or f"Handler for this route lives in the corresponding module under `app/`. The path pattern `{path}` is declared with `@router.{method.lower()}('{path}')`.",
        ...
        "h2_c": auth_hist
        or "The auth tier reflects the principle of least privilege: pre-login endpoints are strictly limited to account creation and recovery; everything behind a JWT sees a real user; admin endpoints show a badge in the UI so misuse is observable.",
        "h3": "Request",
        "h3_a": request_desc,
        "h3_f": request_schema,
        "h4": "Response",
        ...
        "h5": "Errors",
        "h5_a": errors_desc
        or "Standard Vortex error envelope `{error, status, path}`. `401` on bad credentials; `403` on insufficient privilege; `422` on validation; `429` on rate limit; `5xx` on internal error.",
        "h5_f": errors_table
        or (
        ...
        "h6_f": example_curl
        or f"curl -X {method} 'https://node.example{path}' \\\n  -H 'Content-Type: application/json' \\\n  -H 'Authorization: Bearer $TOKEN'",
    }
...
AUTH_GROUP = {
    ...
    "challengeVerify": simple_route(
        "POST",
        "/api/authentication/challenge/verify",
    ...
    "passkeyBegin": simple_route(
        "POST",
        "/api/authentication/passkey/begin",
    ...
    "passkeyFinish": simple_route(
        "POST",
        "/api/authentication/passkey/finish",
...
ROOMS_GROUP = {
    ...
    "messages": simple_route(
        "GET",
        "/api/rooms/{id}/messages?since=&limit=",
    ...
    "draftPut": simple_route(
        "PUT",
        "/api/rooms/{id}/draft",
        "Save draft",
        "Upserts `drafts(room_id, user_id, text, updated_at)`. Draft is server-visible plaintext — deliberately, so it syncs across devices.",
    ),
    "draftGet": simple_route(
        "GET", "/api/rooms/{id}/draft", "Fetch saved draft", "Returns the draft text or 204 if none."
    ),
    "draftDelete": simple_route(
        "DELETE", "/api/rooms/{id}/draft", "Clear draft", "Deletes the row. Typical when the user sends the message."
    ),
    "pin": simple_route(
        "POST",
        "/api/rooms/{id}/pin",
...
MESSAGES_GROUP = {
    ...
    "unreact": simple_route(
        "DELETE",
        "/api/messages/{id}/react/{emoji}",
        "Remove reaction",
...
FILES_GROUP = {
    ...
    "resumeChunk": simple_route(
        "PUT",
        "/api/files/resumable/{id}/chunk/{offset}",
        "Upload a chunk",
...
ADMIN_GROUP = {
    ...
    "userUnsuspend": simple_route(
        "POST",
        "/api/admin/user/{id}/unsuspend",
        "Re-enable account",
...
def term(
    ...
    return {
        ...
        "h2_a": usage
        or "This term appears throughout the codebase and documentation; grep for the lowercase form in `app/` and the language clients to see every call site.",
        "h3": "Related terms",
...
GLOSSARY = {
    ...
    "argon2id": term(
        "Argon2id",
        "Memory-hard password hash (RFC 9106).",
    ...
    "controller": term(
        "Controller",
        "Vortex's attestation + discovery service.",
    ...
    "doubleRatchet": term(
        "Double Ratchet",
        "Signal's per-DM forward-secrecy protocol.",
    ...
    "ech": term(
        "ECH",
        "Encrypted Client Hello (RFC 9460).",
    ...
    "ed25519": term(
        "Ed25519",
        "EdDSA signature over edwards25519 (RFC 8032).",
    ...
    "fanout": term(
        "Fan-out",
        "Broadcasting an event from one source to many subscribers.",
    ...
    "federation": term(
        "Federation",
        "Mutual-trust message delivery between independent Vortex nodes.",
    ...
    "gossip": term(
        "Gossip",
        "Periodic peer-list exchange between Vortex nodes.",
    ...
    "mailbox": term(
        "Mailbox",
        "Recipient-addressed storage slot in BMP.",
    ...
    "nonce": term(
        "Nonce",
        "Number used once — a one-time-use value.",
    ...
    "prekey": term(
        "Prekey",
        "Ephemeral pubkey published ahead of time for bootstrap of new DMs.",
    ...
    "reality": term(
        "Reality",
        "TLS-within-TLS masquerade stealth transport.",
    ...
    "snowflake": term(
        "Snowflake",
        "Tor volunteer-relay transport over WebRTC.",
    ...
    "tls13": term(
        "TLS 1.3",
        "Transport Layer Security, version 1.3 (RFC 8446).",
    ...
    "turn": term(
        "TURN",
        "Traversal Using Relays around NAT (RFC 8656).",
