#!/usr/bin/env python3
"""Генератор golden-векторов NaiveProxy Python ↔ Rust.

Повторный запуск даёт байт-в-байт идентичный файл.

Запуск (из корня репозитория):
    python scripts/gen_naive_parity_vectors.py
"""
...
OUT_PATH = os.path.join(ROOT, "app", "tests", "vectors", "naive_parity.json")

_spec = importlib.util.spec_from_file_location(
    "naive_parity_reference",
    os.path.join(ROOT, "app", "tests", "naive_parity_reference.py"),
)
_reference = importlib.util.module_from_spec(_spec)
sys.modules["naive_parity_reference"] = _reference
