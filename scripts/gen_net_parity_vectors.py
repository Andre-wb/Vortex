#!/usr/bin/env python3
"""Генератор golden-векторов UDP-обнаружения Python ↔ Rust.

Повторный запуск даёт байт-в-байт идентичный файл.

Запуск (из корня репозитория):
    python scripts/gen_net_parity_vectors.py
"""
...
OUT_PATH = os.path.join(ROOT, "app", "tests", "vectors", "net_parity.json")

_spec = importlib.util.spec_from_file_location(
    "net_parity_reference",
    os.path.join(ROOT, "app", "tests", "net_parity_reference.py"),
)
_reference = importlib.util.module_from_spec(_spec)
sys.modules["net_parity_reference"] = _reference
