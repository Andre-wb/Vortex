...
| `adr/`                | Core developer, reviewer | Architecture Decision Records. `001` — message-encryption protocol versioning (`enc_v` registry) and the Double Ratchet migration target. `002` — multi-device v2 (per-device Sesame) decision fork and proposal. |
